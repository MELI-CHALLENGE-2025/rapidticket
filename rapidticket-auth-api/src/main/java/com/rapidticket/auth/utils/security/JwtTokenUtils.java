...
public class JwtTokenUtils {
    ...
    public String generateToken(String subject, List<EnumRoleUser> roles) {
        ...
        List<String> roleNames = roles.stream().map(EnumRoleUser::name).toList();

        return Jwts.builder().setSubject(subject).setIssuedAt(now).setExpiration(expiryDate).addClaims(Map.of("roles", roleNames)).signWith(key, SignatureAlgorithm.HS256).compact();
    }
}
